...
from DARWIN.social_platform.config import Neo4jConfig
...
class AgentGraph:
    ...
    def add_agent(self, agent: SocialAgent):
        if self.backend == "igraph":
            self.graph.add_vertex(str(agent.agent_id))
        else:
    ...
    def add_edge(self, agent_id_0: int, agent_id_1: int):
        try:
            if self.backend == "igraph":
                self.graph.add_edge(str(agent_id_0), str(agent_id_1))
            else:
    ...
    def remove_agent(self, agent: SocialAgent):
        if self.backend == "igraph":
            self.graph.delete_vertices(str(agent.agent_id))
        else:
    ...
    def remove_edge(self, agent_id_0: int, agent_id_1: int):
        if self.backend == "igraph":
            if self.graph.are_adjacent(str(agent_id_0), str(agent_id_1)):
                self.graph.delete_edges([(str(agent_id_0), str(agent_id_1))])
        else:
    ...
    def get_agents(self) -> list[tuple[int, SocialAgent]]:
        if self.backend == "igraph":
            return [(int(node.index), self.agent_mappings[int(node.index)])
    ...
    def get_edges(self) -> list[tuple[int, int]]:
        if self.backend == "igraph":
            return [(int(edge.source), int(edge.target)) for edge in self.graph.es]
        else:
